    ...
    \begin{center}
        \begin{tabular}{llll}
            ...
            \begin{itemize}
                ...
                \customFigure[0.25]{../../Images/L12_7.png}{}
                \vspace{-1em}
            \end{itemize}} \\
            \midrule
            ...
            \begin{itemize}
                ...
                \customFigure[0.25]{../../Images/L12_8.png}{}
            \end{itemize}} \\
            \bottomrule
        \end{tabular}
    \end{center}
...
\begin{summary}
    \begin{center}
        \begin{tabular}{llll}
            ...
            \textbf{Algorithm} & \textbf{Inputs} & \textbf{Outputs} & \textbf{Equations} \\
            \toprule
            Simple GNN & $U_n, V_n, E_n$ & $U_{n+1}, V_{n+1}, E_{n+1}$ & $f_{U_n}, f_{V_n}, f_{E_n}$ \\
            \multicolumn{4}{p{\linewidth}}{
            \begin{itemize}
                \item Graph in, graph out, optimizable, parametrized transformation.
                \item $f$: Update function (e.g. MLP)
                \item $U$: globals, $V$: vertices, $E$: edges. 
                \customFigure[0.75]{../Images/L12_14.png}{}
            \end{itemize}} \\
            \midrule 
            Graph Convolution & $U_n, V_n, E_n$ & $U_{n+1}, V_{n+1}, E_{n+1}$ & $\rho_{V_n \rightarrow V_n}, f_{U_n}, f_{V_n}, f_{E_n}$ \\
            via Adj Matrix & $X, A$ & $X'$ & $GCN(X, A) = \text{act}\left(D^{-1/2} (A + I) D^{-1/2} X W \right) = \text{act}(\tilde{A} X W)$ \\
            \multicolumn{4}{p{\linewidth}}{
            \begin{itemize}
                \item GNN + Add step where we pool info from neighboring nodes. 
                \customFigure[0.75]{../Images/L12_22.png}{}
...
\begin{summary}
    \begin{center}
        \begin{tabular}{llll}
            ...
            \textbf{Algorithm} & \textbf{Inputs} & \textbf{Outputs} & \textbf{Equations} \\
            \toprule
            Graph Nets & $U_n, V_n, E_n$ & $U_{n+1}, V_{n+1}, E_{n+1}$ & $f_{U_n}, f_{V_{i,n}}, f_{E_{k,n}}$ \\
            \multicolumn{4}{p{\linewidth}}{
                \begin{itemize}
                    \customFigure[0.75]{../../Images/L12_5.png}{}
                    \item GNN, but more generalized where we can explore multiple ways of aggregating, updating, and passing information. 
                \end{itemize}} \\
            \midrule
            Graph Attention (GATs) & & & \\
            \multicolumn{4}{p{\linewidth}}{
            \begin{itemize}
                \item \textbf{Overview:} Dynamically compute attention weights for neighbor aggregation.
                \customFigure[0.75]{../../Images/L14_5.png}{}
                \begin{itemize}
                    \item \textbf{Attention:} Computed for all nodes in the graph, then masked out.
                    \begin{itemize}
                        \item Mask is applied so only \textbf{neighbors} are considered.
                    \end{itemize}
                    \item \textbf{Result:} Pure yellow node is updated w/ a new node value that is bright yellow w/ different values for each node relationships. 
                    \begin{itemize}
                        \item Only $w_1,w_2,w_3$ b/c $w_4$ is masked out.
                    \end{itemize}
                \end{itemize}
                \item \textbf{Self-Attention:} GAT $\rightarrow$ Self-Attention when mask is removed. 
            \end{itemize}} \\
            \midrule
            Transformers & & & \\
            \bottomrule
...
\begin{summary}
    ...
    \begin{itemize}
        \item \textbf{MLP:} Permutation Invariance for Neural Networks (functions unaffected by input permutation)
        \customFigure[0.5]{../Images/L9_1.png}{}
        \item \textbf{Deep Sets:} Permutation Invariant NNs (function of element embeddings and pooled features).
        ...
        \item \textbf{GNN}
        \customFigure[0.5]{../../Images/L12_1.png}{}
    \end{itemize}
\end{summary}
\newpage
