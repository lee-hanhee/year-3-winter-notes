...
\begin{paracol}{3}
    ...
    *Jacobian: $\frac{\partial f}{\partial x} (\bar{x})  = \begin{bmatrix} \frac{\partial f}{\partial x_1} (\bar{x})  & \ldots & \frac{\partial f}{\partial x_{n_1}} (\bar{x}) \end{bmatrix} \in \mathbb{R}^{n_2 \times n_1}$ 
    ...
    \textcolor{red}{\textbf{Laplace Transform}} Given a fcn $f: \mathbb{R}_+ \to \mathbb{R}^n$, its Laplace transform is 
    $F(s) = \mathcal{L} \{ f(t) \} := \int_{0^-}^{\infty} f(t) e^{-st} \, dt$, $s \in \mathbb{C}$. \\ 
    *$\mathcal{L}: f(t) \mapsto F(s)$, $t \in \mathbb{R}_+$ (time domain) \& $s \in \mathbb{C}$ (Laplace domain).
    
    \textcolor{blue}{\textbf{P.W. CTS:}} A fcn $f: \mathbb{R}_+ \to \mathbb{R}^n$ is \textbf{p.w. cts} if on every finite interval of $\mathbb{R}$, $f(\cdot)$ has at most a finite \# of discontinuity points ($t_i$) and the limits 
    ...
    \textcolor{blue}{\textbf{Existence of LT Thm}} If $f(\cdot)$ is p.w. cts and of exp. order w/ constants $K, \rho, T > 0$, then $F(\cdot)$ exists and is defined $\forall \, s \in D := \{s \in \mathbb{C} : \text{Re}(s) > \rho\}$ and $F(\cdot)$ is analytic on $D$. \\
    *$D$: Region of convergence (ROC), open half plane. \\
    ...
    \textbf{Convolution:} Let $(f * g)(t) := \int_0^t f(\tau) g(t-\tau) d\tau$, then \\ 
    ...
    \textbf{Shift in $s$:} $\mathcal{L} \{e^{at} f(t)\} = F(s-a) = \mathcal{L} \{f(t)\}\big|_{s \to s-a}$, where $F(s) = \mathcal{L} \{f(t)\}$ \& $a ~ \text{const}$. \\   
    
    \textcolor{red}{\textbf{Cauchy's Residue THM}}
