...
\begin{paracol}{3}
    ...
    *\textbf{Use:} Good to use when $\underline{Z}$ is indep. 
    ...
    \textcolor{red}{\textbf{Linear Regression:}} Estimate unknown target fn $Y=g(\underline{X})$ w/ \\ 
    $\hat{y} = h(\underline{x}) = \underline{w}^T \underline{x} + Z$ \\
    ...
    *$Y$: Target/observed output 
    ...
    \textcolor{blue}{\textbf{ML Estimator:}} Given iid obs. $\{(\underline{x}_1, y_1), \ldots, (\underline{x}_n, y_n)\}$, estimate $\underline{w}$ \\
    $\hat{\underline{w}}_{\text{ML}} = (X X^T)^{-1} X^T \underline{y}$ \\
    ...
    *$X^\dagger = (X^T X)^{-1} X^T$: Pseudo-inverse of $X$ (minimizes $||X \underline{w} - \underline{y}||_2^2 \iff$ maximizes the likelihood of training data) \\
    ...
    \textcolor{green}{\textbf{Non-Linear Trans:}} $\hat{y} = \underline{w}^T \underline{\phi}(\underline{x}) + Z$ \\
    *$\underline{\phi}(\underline{x})$: Non-linear transformation of $\underline{x}$ \\
    ...
    Given iid obs. $\{(\underline{x}_1, y_1), \ldots, (\underline{x}_n, y_n)\}$, estimate $\underline{w}$ \\
    $\hat{\underline{w}}_{\text{ML}} = (X X^T)^{-1} X^T \underline{y}$ \\
    ...
    *Overfitting: Model too complex, low bias, high variance. \\
    ...
    $\hat{\underline{w}}_{\text{MAP}} = (X X^T + \lambda I)^{-1} X^T \underline{y}$ \\
