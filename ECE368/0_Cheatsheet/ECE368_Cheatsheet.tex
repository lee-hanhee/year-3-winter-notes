...
\begin{paracol}{3}
    ...
    *$P[a \leq X \leq b] = \int_a^b f_X(x) \, dx$ IS THIS CORRECT?
    ...
    \textcolor{blue}{\textbf{Exp.:}} $E[h(X)] = \int_{-\infty}^{\infty} h(x) f_X(x) dx \mid \sum_{k=-\infty}^{\infty} h(k) f_X(k)$ \\

    \textcolor{darkgreen}{\textbf{Variance:}} $\text{Var}[X] = E[(X - \mu)^2] = E[X^2] - E[X]^2$
    ...
    *Given a sequence of i.i.d. RVs, $X_1, X_2, \ldots, X_n$, $M_n$ is unbiased and consistent.
    ...
    \textcolor{blue}{\textbf{Weal Law of Large \#s}} $\lim_{n \to \infty} P[|M_n - \mu| < \epsilon] = 1 \; \forall \epsilon > 0$.
    ...
    *Discrete $X$: $\hat{\Theta} = \arg \max_\theta P_{\underline{X}} (\underline{x} | \theta) \overset{\text{log}}{=} \arg \max_\theta \sum_{i=1}^n \log P_X(x_i | \theta)$ \\
    *Cont. $X$: $\hat{\Theta} = \arg \max_\theta f_{\underline{X}} (\underline{x} | \theta) \overset{\text{log}}{=} \arg \max_\theta \sum_{i=1}^n \log f_X(x_i | \theta)$
