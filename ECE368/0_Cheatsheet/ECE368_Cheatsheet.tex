...
\begin{paracol}{3}
    ...
    *Subscript indicates the RV, and the value indicates the realization.  
    ...
    \textcolor{blue}{\textbf{Additive Rule:}} $P(A \cup B) = P(A) + P(B) - P(A \cap B)$ 
    ...
    *$P(A \cap B) = P(A|B) P(B) = P(B|A) P(A)$
    ...
    \textcolor{darkgreen}{\textbf{Partition:}} $H_1,\ldots,H_n$ is a partition if \\
    (1) $H_i \cap H_j = \emptyset$ for $i \neq j$, (2) $H_1 \cup H_2 \cup \ldots \cup H_n = \Omega$.
    ...
    *$P[a \leq X \leq b] = \int_a^b f_X(x) \, dx$ 
    ...
    \textcolor{blue}{\textbf{Exp.:}} \\ 
    *$E[h(X)] \text{=} \int_{-\infty}^{\infty} h(x) f_X(x) dx$ \\
    *$E[h(X)]= \sum_{k=-\infty}^{\infty} h(k) P_X(x_i \text{=} k)$ \\
    *$E[aX+b] = aE[X] + b$ 

    \textcolor{blue}{\textbf{Variance:}} $\sigma_X^2 = \text{Var}[X] = E[(X - E[X])^2] = E[X^2] - E[X]^2$ \\
    ...
    *$P_{Y|X}(y|x) = \frac{P_{X,Y}(x, y)}{P_X(x)} = \frac{P_{X|Y}(x|y) P_Y(y)}{\sum_{j=1}^\infty P_{X|Y}(x|y_j) P_Y(y_j)}$ 
    ...
    $P_{\Theta | \underline{X}} (\theta | \underline{x}) = \begin{cases}
    ...
    *$P_{\underline{X}} (\underline{x}) = \sum_{j=1}^{\infty} P_{\underline{X} | \Theta} (\underline{x} | \theta_j) P_\Theta (\theta_j)$
    ...
    *$E[Y \mid X] = \int_{-\infty}^{\infty} y f_{Y|X} (y|x) \, dy$ \\
    *$E[E[Y|X]]$ w.r.t. $X \mid E[Y|X]$ w.r.t. $Y$. 

    \textcolor{blue}{\textbf{Q Fcn:}} $Q(x) = \frac{1}{\sqrt{2\pi}} \int_x^\infty e^{-t^2/2} \, dt$ 

    \textcolor{blue}{\textbf{Counting:}} 
    
    \textcolor{darkgreen}{\textbf{Permutations:}} $P(n, k) = \frac{n!}{(n-k)!}$ \\
    *Order matters

    \textcolor{darkgreen}{\textbf{Combinations:}} $\binom{n}{k} = \frac{n!}{k!(n-k)!}$ \\
    *Order doesn't matter

    \textcolor{darkgreen}{\textbf{Multinomial Coeff.:}} $\binom{n}{n_1,\ldots,n_m} = \frac{n!}{n_1! \ldots n_m!}$ \\
    ...
    *$\beta(R) = P[\underline{X} \in R^c \mid H_1]$ (missed detection)
    ...
    \textcolor{darkgreen}{\textbf{Prob. of Error:}} $P = \alpha \pi_0 + \beta (1-\pi_0)$ \\
    ...
    *\textbf{No Order:} Simplifies, but no way to order the RVs, so lose directivity, lose info.
    ...
    % \textcolor{blue}{\textbf{Cond Indep.}} 2 \textbf{non-neighbouring} nodes (RVs) are conditionally indep. given the set of nodes that separate them: \\
    ...
    *\textbf{Note:} Can be continuous (density).
    ...
    (1) $Z_n$ nodes are H-T or T-T and $Z_1,\ldots,Z_N$ are unobserved\\
    $\implies$ No indep. among $X_n$'s, also $\{X_n\}$ are not a MC.

    (2) Latent var. $Z_1,\ldots,Z_N$ are MC $\implies$ $Z_{n+1} \perp Z_{n-1} \mid Z_n$ \\
    $\implies$ e.g. $\{X_1,X_2\} \perp \{X_3,\ldots,X_N\} \mid \{Z_3\}$

    \textcolor{blue}{\textbf{Common Problems}} \\ 
    1. Given HMM, find $P(\underline{x})$ for any $\underline{x} = \{x_1,\ldots,x_N\}$ \\
    2. Given HMM/$\underline{x}$, find most likely $z_n$ \\
    3. Given HMM/$\underline{x}$, find most likely seq. of states $\underline{z} = \{z_1,\ldots,z_N\}$ 

    \textcolor{red}{\textbf{Message Passing Algos: Given HMM, find $P(\underline{x}) \; \forall \underline{x}$}} \\
    \textcolor{blue}{\textbf{Forward:}} \\
    \textcolor{green}{\textbf{Defn of $\alpha$:}} \\
    ...
    \textcolor{green}{\textbf{Recursion:}} \\
    $\boxed{\alpha(z_1) = P(x_1,z_1) = P(z_1)P(x_1 \mid z_1)}$ \\
    $\boxed{\alpha(z_n) = P(x_n \mid z_n) \sum_{z_{n-1}} P(z_n \mid z_{n-1}) \alpha(z_{n-1})}$ \\
    $\boxed{P(\underline{x}) = \sum_{z_N} \alpha(z_N)}$ \\
    \textcolor{green}{\textbf{Algo:}} $\{\alpha(z_n)\}_{z_n = 1,\ldots,K}$: Message from $Z_n$ to $Z_{n+1}$ \\
    *\textbf{Complexity:} $O(K^2 N)$ \\
    *$O(K)$ for each $\alpha(z_n)$, $O(K^2)$ for message at time $n$

    \textcolor{blue}{\textbf{Backward:}} \\
    \textcolor{green}{\textbf{Defn of $\beta$:}} \\
    ...
    \textcolor{green}{\textbf{Recursion:}} \\
    $\boxed{\beta(z_N) = 1 \; \forall z_n}$ \\
    $\boxed{\beta(z_n) = \sum_{z_{n+1}} P(x_{n+1} \mid z_{n+1}) P(z_{n+1} \mid z_n) \beta(z_{n+1})}$ \\
    $\boxed{P(\underline{x}) = \sum_{z_1} P(z_1) P(x_1 \mid z_1) \beta(z_1)}$ \\
    \textcolor{green}{\textbf{Algo:}} $\{\beta(z_n)\}_{z_n = 1,\ldots,K}$: Message from $Z_{n-1}$ to $Z_n$ \\
    *\textbf{Complexity:} $O(K^2 N)$ \\
    *$O(K)$ for each $\beta(z_n)$, $O(K^2)$ for message at time $n$

    \textcolor{blue}{\textbf{Forward Backward (Same Time):}} 
    
    $\alpha (z_n) \beta(z_n) = P(\underline{x}, z_n) \implies P(\underline{x}) = \sum_{z_n} \alpha(z_n) \beta(z_n) \; \forall n$ 

    \textcolor{red}{\textbf{Given HMM and $\underline{x}$, find most likely $z_n$}} 

    \textcolor{blue}{\textbf{Defn of $\gamma$:}} \\
    $\gamma (z_n) \equiv P(z_n \mid \underline{x}), \; 1 \leq n \leq N$ 

    \textcolor{blue}{\textbf{Solution:}} \\
    $\gamma(z_n) = \frac{\alpha(z_n) \beta(z_n)}{\sum_{z_n'} \alpha(z_n') \beta(z_n')}$ \\
    $z_n^* = \arg \max_{z_n} \gamma(z_n)$ \\
    *\textbf{Complexity:} $O(K^2 N)$ 

    \textcolor{blue}{\textbf{Scaling Problem:}} $\alpha/\beta$ can be small for large/small $n$ \\ 
    \textcolor{green}{\textbf{1. Forward:}} \\
    \textcolor{purple}{\textbf{Defn of $\hat{\alpha}$:}} \\
    $\hat{\alpha}(z_n) \equiv \frac{\alpha(z_n)}{P(x_1,\ldots,x_n)} = P(z_n \mid x_1,\ldots,x_n)$ \\
    *Does not shrink as $n \uparrow$ \\
    \textcolor{purple}{\textbf{Defn of $c_n$:}} \\
    $c_n \equiv P(x_n \mid x_1,\ldots,x_{n-1})$ \\
    \textcolor{purple}{\textbf{Recursion:}}\\
    ...
    $\boxed{P(x_1,\ldots,x_n) = \prod_{m=1}^N c_m}$

    \textcolor{green}{\textbf{2. Backward:}} \\
    \textcolor{purple}{\textbf{Defn of $\hat{\beta}$:}} \\
    ...
    *Have to fwd, then bwd pass. \\
    ...
    1. $\hat{\alpha}(z_1) = \frac{1}{c_1} P(z_1) P(x_1 \mid z_1) $ \\
    Fwd message passing to compute $\hat{\alpha}(z_n)$ and $c_n$, $2 \leq n \leq N$ \\
    ...
    \textcolor{red}{\textbf{Viterbi Algo: Given HMM and $\underline{x}$, find most likely $\underline{z}$}} \\
    $\hat{\underline{z}} = \arg \max_{\underline{z}} P(\underline{z} \mid \underline{x}) = \arg \max_{\underline{z}} P(\underline{x}, \underline{z})$?

    \textcolor{blue}{\textbf{Dynamic Programming:}} Path length $- \log P(\underline{x}, \underline{z})$ \\ $= - \left[\log P(z_1) + \sum_{n=2}^N \log P(z_n \mid z_{n-1}) + \sum_{n=1}^N \log P(x_n \mid z_n)\right]$ \\
    ...
    \textcolor{blue}{\textbf{Current Highest Log-Prob to State $z_n$:}} \\
    ...
    *$w_{n+1}(z_{n+1}) = \max_{z_n} w_n(z_n) + \log P(z_{n+1} \mid z_n) + \log P(x_{n+1} \mid z_{n+1})$ \\
    ...
    \textcolor{blue}{\textbf{Trellis Diagram of MC}} Unfolding of MC over time 

    \textcolor{blue}{\textbf{Viterbi Algorithm:}} \\
    ...
    % 2. Termination: $\max_{z_N} w_N(z_N)$ \\
    % 4. Backtrack: $\hat{z}_N = \arg \max_{z_N} w_N(z_N)$ \\
    % For $n=N-1$ to $1$: $\hat{z}_n = \arg \max_{z_n} \left[ w_n(z_n) + \log P(z_{n+1} \mid z_n) + \log P(x_{n+1} \mid z_{n+1})\right]$ \\
    % 5. $\hat{\underline{z}} = \{\hat{z}_1,\ldots,\hat{z}_N\}$
