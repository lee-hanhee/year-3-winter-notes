...
\begin{paracol}{3}
    ...
    *Independent: $f_{\underline{X}} (\underline{x}) = \prod_{i=1}^n f_{X_i} (x_i) = \frac{1}{(2\pi)^{n/2} \prod_{i=1}^n \sigma_i} e^{-\frac{1}{2} \sum_{i=1}^n \left( \frac{x_i - \mu_i}{\sigma_i} \right)^2}$
    *$\text{Indep. Gaus.} \overset{\text{Indep.}}{\implies} \text{Jointly Guas.} \overset{\text{L.T}}{\implies} \text{Jointly Guas.}$ W6 L1 \\
    *Conditional PDF of Gaus. RVs is Gaussian. W6 1 \\
    ...
    *Bin. hyp. dec. boundary: Quadratic in space of $\underline{x}$
    ...
    \textcolor{red}{\textbf{Guassian Estimation:}} \textcolor{blue}{\textbf{ML Estimator for $\theta$ w/ Indep. Guas:}}  \\
    ...
    *$X_i \text{=} \theta + Z_i$: Measurement \\ 
    *$Z_i \sim \mathcal{N}(0, \sigma_i^2)$: Noise (indep. not iid) \\
    ...
    *\textbf{SC:} If $\sigma_i^2 = \sigma^2 \; \forall i$ (iid), then $\hat{\theta}_{\text{ML}} = \frac{1}{n} \sum_{i=1}^n x_i$.
    ...
    \textcolor{blue}{\textbf{MAP Estimator for $\theta$ w/ Indep. Gaus., Gaus. Prior:}} \\
    ...
    *\textbf{Reducing Uncertainty:} 2nd term is PSD, so given $\underline{Y} = \underline{y}$, always reducing uncertainty in $\underline{X}$.
    ...
    \textcolor{blue}{\textbf{LMMSE Estimator for $\underline{X}$ Given $\underline{Y}$ w/ non-Guas. $\underline{X}$, $\underline{Y}:$}} \\
