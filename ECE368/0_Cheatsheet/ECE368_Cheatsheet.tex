...
\begin{paracol}{3}
    ...
    *Bin. hyp. dec. boundary: Quadratic in space of $\underline{x}$
    ...
    Given $\underline{X} = \{X_1,\ldots,X_n\}$, $X_i = \theta + Z_i$, $Z_i \sim \mathcal{N}(0, \sigma^2)$ \\
    $\hat{\theta}_{\text{ML}} = \frac{\sum_{i=1}^n \frac{x_i}{\sigma_i^2}}{\sum_{i=1}^n \frac{1}{\sigma_i^2}}$ (weighted avg. of $\underline{x}$) \\
    *$\frac{1}{\sigma_i^2}$: Precision of $X_i$ \\
    *Larger $\sigma_i^2 \implies$ less weight on $X_i$ (less reliable measurement) \\
    *If $\sigma_i^2 = \sigma^2 \; \forall i$, then $\hat{\theta}_{\text{ML}} = \frac{1}{n} \sum_{i=1}^n x_i$ (sample mean) 
    ...
    *Gaussian prior $f_\Theta$ is equivalent to a prior meas. $x_0$ w/ $\sigma_0^2$.
    ...
    * 
    ...
    \textcolor{blue}{\textbf{MAP Estimator for $\underline{X}$ Given $\underline{Y}$:}} $\underline{W} = (\underline{X},\underline{Y}) \sim \mathcal{N} (\underline{\mu}, \Sigma)$ \\
