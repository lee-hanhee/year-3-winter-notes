...
\begin{paracol}{3}
    ...
    *$O(K)$ for each $\beta(z_n)$, $O(K^2)$ for message at time $n$
    ...
    \textcolor{green}{\textbf{Forward Backward:}} $\alpha (z_n) \beta(z_n) = P(\underline{x}, z_n)$ \\
    $P(\underline{x}) = \sum_{z_n} \alpha(z_n) \beta(z_n) \; \forall n$ 

    \textcolor{blue}{\textbf{Viterbi Algo:}} Given HMM and $\underline{x}$, find most likely $z_n$\\
    ...
    *\text{Complexity:} $O(K^2 N)$ \\
    ...
    \textcolor{green}{\textbf{Scaling:}} \\
    *$\alpha(z_n),\beta(z_n)$ can be small for large/small $n$ \\ 
    *Cannot use log b/c of sum operation. \\
    $\hat{\alpha}(z_n) = \frac{\alpha(z_n)}{P(x_1,\ldots,x_n)} = P(z_n \mid x_1,\ldots,x_n)$ \\
