...
\begin{paracol}{3}
    ...
    *Have to fwd, then bwd pass. \\
    ...
    \textcolor{blue}{\textbf{Path Length:}} $- \log P(\underline{x}, \underline{z})$ \\ $= - \left[\log P(z_1) + \sum_{n=2}^N \log P(z_n \mid z_{n-1}) + \sum_{n=1}^N \log P(x_n \mid z_n)\right]$ 
    ...
    *\textbf{Base Case:} $w_1(z_1) = \log P(z_1) + \log P(x_1 \mid z_1)$ 
    ...
    *\textbf{Complexity:} $O(K^2 N)$, $O(K^2)$ per step. 
